...
- `LNE` (opcode `4'b1011`) - Submit a framebuffer line draw using the start point from the previous `LNS` and the endpoint from this instruction. Encoded as `LNE Rd, Rs, Rt` where `Rd` is `x1`, `Rs` is `y1`, and `Rt` is the per-line 8-bit RGB-3-3-2 color. `LNE` uses the same framebuffer controller and back-pressure path as `STRFB`.
- `STRFB` (opcode `4'b1100`) - Write a pixel to the framebuffer. Encoded as `STRFB Rd, Rs, Rt` where `Rd` is the framebuffer X coordinate, `Rs` is the framebuffer Y coordinate, and `Rt` is the 8-bit pixel color. The framebuffer stores 8-bit color in **RGB 3-3-2** format (`{R[2:0], G[2:0], B[1:0]}`); the LSU passes `Rt` straight through to the framebuffer (no thresholding), and the VGA scanout bit-replicates each field up to the 8-bit-per-channel DACs. `STRFB` reuses the existing scheduler `REQUEST/WAIT` path through a dedicated framebuffer controller, so it back-pressures the warp the same way `STR` does.
...
3. `REQUEST` - Request data from global memory if necessary (if `LDR`, `STR`, `STRFB`, `LNS`, or `LNE` instruction; `STRFB`/`LNE` issue framebuffer writes through a dedicated controller while reusing the same handshake stalls).
