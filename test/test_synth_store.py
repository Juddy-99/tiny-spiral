...
async def test_pc9_str_handshake(dut):
    ...
    for sample in range(256):
        ...
        lsu_states = [
            _to_int_or_none(gpu.dbg_core0_lsu_state[i].value) for i in range(4)
        ]
        ...
        log_lines.append(
            f"  s={sample} pc={pc} core={core_state} act=0b{active_mask:04b} "
            f"mem_wv=0b{wv:04b} mem_wr=0b{wr:04b} stuck=0b{stuck:04b} "
            f"ram_we=0b{ram_we:04b} lsu_cv=0b{cv:04b} lsu_cr=0b{cr:04b} "
            f"lsu_st={lsu_states}"
        )
